...
function getWeChatModule() {
  ...
  if (Platform.OS !== 'android') {
    console.warn('⚠️ [wechatAndroidAuth] 此模組僅支持 Android 平台');
    return null;
  }
  ...
  try {
    ...
    // 調試信息：檢查模組是否可用
    if (__DEV__) {
      console.log('🔍 [wechatAndroidAuth] 檢查微信原生模組 (Android)...');
    ...
    console.error('❌ [wechatAndroidAuth] 獲取微信模組時發生錯誤:', error);
    return null;
  }
}
...
/**
 * 初始化微信 SDK（支持延遲重試）- Android 專用
 * 在應用啟動時調用，確保微信功能可用
 * 如果初始化失敗，不會拋出錯誤，避免應用崩潰
 */
export function initWeChatSDK(retryCount = 0, maxRetries = 3) {
  if (Platform.OS !== 'android') {
    console.warn('⚠️ [wechatAndroidAuth] initWeChatSDK 僅支持 Android 平台');
  ...
  if (!WeChat) {
    ...
    if (retryCount < maxRetries) {
      console.log(`⏳ [Android] WeChat 模組未找到，${1000 * (retryCount + 1)}ms 後重試 (${retryCount + 1}/${maxRetries})...`);
    ...
    console.warn('⚠️ [Android] WeChat 原生模組未找到，請檢查：');
  ...
  if (typeof WeChat.registerApp !== 'function') {
    console.error('❌ [Android] WeChat.registerApp 方法不存在');
  ...
  console.log(`🔧 [Android] 正在初始化微信 SDK (AppID: ${WX_APP_ID})...`);
  
  return WeChat.registerApp(WX_APP_ID)
    .then((result) => {
      if (result) {
        console.log('✅ [Android] 微信 SDK 初始化成功');
      } else {
        console.warn('⚠️ [Android] 微信 SDK 初始化失敗（registerApp 返回 false）');
        console.warn('   可能的原因：');
    ...
    .catch((error) => {
      console.warn('⚠️ [Android] 微信 SDK 初始化失敗:', error.message);
...
/**
 * 檢查微信是否已安裝 - Android 專用
 */
export async function isWXAppInstalled() {
  if (Platform.OS !== 'android') {
    console.warn('⚠️ [wechatAndroidAuth] isWXAppInstalled 僅支持 Android 平台');
    return false;
  }
  ...
  if (!WeChat) {
    console.warn('⚠️ [Android] WeChat 原生模組未找到');
    return false;
  }

  if (typeof WeChat.isWXAppInstalled !== 'function') {
    console.error('❌ [Android] WeChat.isWXAppInstalled 方法不存在');
    return false;
  }
  ...
  try {
    ...
    console.error('❌ [Android] 檢查微信安裝狀態失敗:', error);
    return false;
  }
}
...
export async function wechatLogin() {
  if (Platform.OS !== 'android') {
    console.warn('⚠️ [wechatAndroidAuth] wechatLogin 僅支持 Android 平台');
    return null;
  }
  ...
  try {
    ...
    // 檢查模組是否可用
    if (!WeChat) {
      console.error('❌ [Android] 微信 SDK 模組未正確初始化，無法使用微信登入');
      console.error('   請檢查：');
    ...
    if (typeof WeChat.sendAuthRequest !== 'function') {
      console.error('❌ [Android] WeChat.sendAuthRequest 方法不存在');
    ...
    if (!registered) {
      console.warn('⚠️ [Android] 微信 SDK 初始化失敗，無法使用微信登入');
      return null;
    }
    ...
    if (!isInstalled) {
      console.warn('⚠️ [Android] 微信應用未安裝');
    ...
    try {
      ...
      if (response && response.code) {
        console.log('✅ [Android] 微信授權成功，code:', response.code);
        return response.code;
      } else {
        console.warn('⚠️ [Android] 微信授權響應異常:', response);
        return null;
      }
    } catch (authError) {
      ...
      if (authError.code === -2 || authError.message?.includes('USER_CANCEL')) {
        // -2 或用戶取消
        console.log('ℹ️ [Android] 用戶取消微信登入');
        return null;
      } else {
        console.warn('⚠️ [Android] 微信授權失敗:', authError.message || authError);
        return null;
      }
    }
  } catch (error) {
    console.error("❌ [Android] WeChat login error:", error);
...
/**
 * 監聽微信響應事件（可選，用於處理其他類型的響應）- Android 專用
 */
export function addWeChatResponseListener(callback) {
  if (Platform.OS !== 'android') {
    console.warn('⚠️ [wechatAndroidAuth] addWeChatResponseListener 僅支持 Android 平台');
    return null;
  }
  ...
  if (!emitter) {
    console.warn('⚠️ [Android] WeChat 事件監聽器未初始化');
    return null;
  }

  return emitter.addListener('WeChat_Resp', (response) => {
    console.log('📱 [Android] 收到微信響應:', response);
    if (callback) {
      callback(response);
    }
  });
